...
const BANK_CODE_REGEX = /^[A-Za-z0-9-]{1,16}$/;
const ACCOUNT_NAME_REGEX = /^[A-Za-z0-9 .,'\-&()]{1,128}$/;
const BANK_NAME_REGEX = /^[A-Za-z0-9 .,'\-&()]{1,128}$/;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const { orgId, bankCode, bankName, accountNumber, accountName, bankCountry } = req.body || {};
    ...
    if (bankCode && (typeof bankCode !== 'string' || !BANK_CODE_REGEX.test(bankCode))) {
        return res.status(400).json({ error: 'Invalid bankCode' });
    }
    if (bankName && (typeof bankName !== 'string' || !BANK_NAME_REGEX.test(bankName))) {
        return res.status(400).json({ error: 'Invalid bankName' });
    }
    const country = bankCountry || 'NG';
