...
const Clients: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing) {
        await updateClient({ ...clientData, id: formData.id, createdAt: new Date().toISOString() }); // Keeping original createdAt would be better but simple for now
    } else {
    ...
    <div className="space-y-6">
                ...
                <Card key={client.id} className="p-6 flex flex-col justify-between hover:shadow-md transition-shadow">
                    <div>
                        <div className="flex justify-between items-start mb-2">
                            ...
                            {client.company && <span className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-600">{client.company}</span>}
                        </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-100 flex justify-end gap-2">
                        ...
                        <Button variant="outline" className="h-8 px-2 text-red-600 border-red-100 hover:bg-red-50" onClick={() => handleDelete(client.id)}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
            <Card className="w-full max-w-2xl p-6 bg-white my-8">
                <h3 className="text-lg font-bold mb-4">{isEditing ? 'Edit Client' : 'Add New Client'}</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-3 mt-6 pt-4 border-t border-slate-100">
                        ...
                        <Button type="submit" isLoading={loading} className="flex-1">Save Client</Button>
                    </div>
                </form>
            </Card>
        </div>
